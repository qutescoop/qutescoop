...
Window::Window(QWidget* parent)
    ...
    actionShowImmediateRoutes->setChecked(Settings::onlyShowImmediateRoutePart());
    connect(
        actionShowImmediateRoutes,
        &QAction::toggled,
        this,
        [this](const bool &newValue) {
            actionShowImmediateRoutes_triggered(newValue);
        }
    );
...
void Window::on_actionHideAllWindows_triggered() {
    ...
    if (StaticSectorsDialog::instance(false) != 0) {
        StaticSectorsDialog::instance()->close();
    }
...
void Window::on_tbZoomOut_customContextMenuRequested(QPoint) {
    mapScreen->glWidget->zoomTo(2.);
}
void Window::on_tbZoomIn_customContextMenuRequested(QPoint) {
    mapScreen->glWidget->zoomTo(2.);
}
